...
import { Bell, LogOut, Settings, Menu, X, User, Sun, Moon, Check, X as XIcon, Clock, Calendar, Scissors } from 'lucide-react';
...
import SalonProfileModal from './SalonProfileModal';
import SalonSettingsModal from './SalonSettingsModal';
...
const SalonHeader = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${SALON_API}/notifications/salon`, {
  ...
  const markAllRead = async () => {
    try {
      const token = localStorage.getItem('silverscissor_token');
      await axios.put(`${SALON_API}/notifications/salon/read-all`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const getNotifIcon = (type) => {
    switch (type) {
      ...
      case 'delay_alert': return <Clock className="w-4 h-4 text-yellow-500" />;
  ...
  return (
    <div className="relative bg-gradient-to-r from-green-500 via-emerald-400 to-teal-500 dark:from-slate-700 dark:via-slate-600 dark:to-slate-700 text-white sticky top-0 z-40 shadow-xl shadow-green-500/20 dark:shadow-slate-900/50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={notifRef}>
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-red-500 border-2 border-green-500 dark:border-slate-700 rounded-full flex items-center justify-center text-[10px] font-bold">
              ...
              {showNotifDropdown && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 max-sm:fixed max-sm:inset-x-4 max-sm:top-20 max-sm:mt-0 max-sm:w-auto bg-white dark:bg-gray-800 rounded-xl shadow-2xl z-50 border border-gray-100 dark:border-gray-700 text-gray-800 dark:text-white max-h-[70vh] flex flex-col">
            ...
            <div className="relative hidden md:block">
              ...
              {showProfileDropdown && (
                <>
                  ...
                  ></div>
                  <div className="absolute right-0 mt-2 w-72 max-sm:max-w-[calc(100vw-1rem)] bg-white dark:bg-gray-800 rounded-xl shadow-2xl py-2 z-50 border border-gray-100 dark:border-gray-700 text-gray-800 dark:text-white">
                    {/* Salon Info */}
      ...
      <SalonProfileModal isOpen={showProfileModal} onClose={() => setShowProfileModal(false)} />
      <SalonSettingsModal isOpen={showSettingsModal} onClose={() => setShowSettingsModal(false)} />
    </div>
  );
};
